...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, u1, layers, dt, lb, ub, q, a, b):
        ...
        self.loss = self.a*tf.reduce_sum(tf.square(self.u0_tf - self.U0_pred)) + \
                    self.b*tf.reduce_sum(tf.square(self.u1_tf - self.U1_pred))
...
if __name__ == "__main__": 
    ...
    for i in range(iter):
        ...
        model = PhysicsInformedNN(x0, u0, x1, u1, layers, dt, lb, ub, q, a, b)
        model.train(nIter = 50000)
        print('this is %d training'% (i))
        print("************Parameters**************")
        print('Para a: %e' % (a))
        print('Para b: %e' % (b))
        ...
        ansnoiseless_l1.append(error_lambda_1)
        ansnoiseless_l2.append(error_lambda_2)
        ...
        ansnoise_l1.append(error_lambda_1_noisy)
        ansnoise_l2.append(error_lambda_2_noisy)
        print("************End**************")
    print("They are results above:")
    print(ansnoiseless_l1)
    print('the average of 10 test noiseless Error l1: %.5f%% ' % (sum(ansnoiseless_l1) / iter))
    print(ansnoiseless_l2)
    print('the average of 10 test noiseless Error l2: %.5f%% ' % (sum(ansnoiseless_l2) / iter))
    print(ansnoise_l1)
    print('the average of 10 test noise Error l1: %.5f%% ' % (sum(ansnoise_l1) / iter))
    print(ansnoise_l2)
    print('the average of 10 test noise Error l2: %.5f%% ' % (sum(ansnoise_l2) / iter))
