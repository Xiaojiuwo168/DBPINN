...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, v0, tb, X_f, layers, lb, ub):
        ...
        # Loss
        self.loss = self.a*(tf.reduce_mean(tf.square(self.u0_tf - self.u0_pred)) + \
                    tf.reduce_mean(tf.square(self.v0_tf - self.v0_pred))) + \
                    self.b*(tf.reduce_mean(tf.square(self.u_lb_pred - self.u_ub_pred)) + \
                    tf.reduce_mean(tf.square(self.v_lb_pred - self.v_ub_pred)) + \
                    tf.reduce_mean(tf.square(self.u_x_lb_pred - self.u_x_ub_pred)) + \
                    tf.reduce_mean(tf.square(self.v_x_lb_pred - self.v_x_ub_pred))) + \
                    self.c*(tf.reduce_mean(tf.square(self.f_u_pred)) + \
...
if __name__ == "__main__": 
    ...
    b = np.random.uniform(0.8,1.3)
    c = np.random.uniform(0.8,1.3)
